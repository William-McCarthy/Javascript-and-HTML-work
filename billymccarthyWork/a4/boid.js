...
let Boid = class {
	constructor(flock, position, velocity, lovebird) {
		this.flock = flock
	...
	toString() {
		return `Boid${this.idNumber} p:(${this.position.toFixed(2)})  v:(${this.velocity.toFixed(2)}) l:(${this.lovebird.toFixed(2)})`
	}
	...
	calculateForces(t, dt) {
		...
		this.flock.boids.forEach(boid => {
			if (boid !== this) {
				...
				let d = offset.magnitude
				let range = 50
				if (this.idNumber % 2 == 0 && boid.idNumber % 2 == 0){
					this.forces.lovebird.addMultiples(-offset + 12)
