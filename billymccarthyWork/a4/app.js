...
const billysParticleStartCount = 0
let billysParticles = []
for (var i = 0; i < 20; i++){
	let pt = new BillyParticle()
	billysParticles.push(pt)
}


//an object to hold boids
 const boidParticlesStartCount = 20
	...
	mainP5 = new p5(
		...
		function(p) {
			...
			p.setup = () => {
				...
				createSlider({label:"BillyNoiseScale", min:.1, max: 4, defaultValue: .4, step: .1})
				createSlider({label:"BillyWiggleForce", min:.1, max: 4, defaultValue: .4, step: .1})
				createSlider({label:"BillyDrag", min:.001, max: .1, defaultValue: .014, step: .001})
				createSlider({label:"BillyBorder", min:1, max: 100, defaultValue: 30, step: 1})
			...
			p.draw = () => {
				...
				// UPDATE!
				if (!paused) {
					billysParticles.forEach(pt => pt.update(t, dt))
				...
				// Draw KatesParticles
				billysParticles.forEach(pt => pt.draw(p))
				if (FLAGS.drawKatesDebug)
					billysParticles.forEach(pt => pt.drawDebug(p))
